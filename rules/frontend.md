...
Auto-generated API client from the backend's `openapi.yaml` spec is placed into `src/Api/`. DO NOT EDIT — files are gitignored and regenerated by `nix-shell` (which runs `openapi-generator-cli`). If `Api/Data.elm` is outdated after changing `openapi.yaml`, run `nix-shell` to regenerate it.
